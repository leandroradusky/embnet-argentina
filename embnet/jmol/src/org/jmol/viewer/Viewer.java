/* $RCSfile$
 * $Author: hansonr $
 * $Date: 2011-05-29 01:32:52 +0200 (dim., 29 mai 2011) $
 * $Revision: 15508 $
 *
...
public class Viewer extends JmolViewer implements AtomDataServer {
  ...
  void rotateSelected(float deltaX, float deltaY, BitSet bsSelected) {
    if (isJmolDataFrame())
      return;
    if (mouseEnabled) {
      transformManager.rotateXYBy(deltaX, deltaY, setMovableBitSet(bsSelected, true));
...
//      fileManager.setFileInfo(saveInfo);
  ...
  Object getImageAs(String type, int quality, int width, int height,
    ...
    if (c == null) {
      Image eImage = getScreenImage(null);
      if (eImage != null) {
        try {
          if (quality < 0)
            quality = 75;
  ...
  public void moveSelected(int deltaX, int deltaY, int x, int y,
                           BitSet bsSelected, boolean isTranslation, boolean dragAtoms) {
  ...
  void undoAction(int taintedAtom, int type, boolean clearRedo) {
    ...
    case ACTION_UNDO:
      ...
      s = (String) actionStatesRedo.get(0);
